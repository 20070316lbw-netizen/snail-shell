...
def _load_data(ctx) -> tuple:
    ...
    with DataLoader() as loader:
        ...
        def _zscore(group):
            ...
            group[feature_cols] = (group[feature_cols] - roll.mean()) / (
                roll.std() + 1e-8
            )
            return group
    ...
    def _slice(df, s, e):
        ...
        sub = df[mask].copy()
        return sub[feature_cols].values.astype(np.float32), sub[
            label_col
    ...
    log.info(
        f"   Train   : {Xtr.shape[0]:>6} 条   {split['train_start']} ~ {split['train_end']}"
    )
    log.info(
        f"   Val Q1  : {Xvq1.shape[0]:>6} 条   {split['val_q1_start']} ~ {split['val_q1_end']}"
    )
    log.info(
        f"   Val Q2-4: {Xv.shape[0]:>6} 条   {split['val_q2_4_start']} ~ {split['val_q2_4_end']}"
    )
    log.info(
        f"   Test    : {Xte.shape[0]:>6} 条   {split['test_start']} ~ {split['test_end']}"
    )
...
def _print_table(df: pd.DataFrame, title: str = ""):
    ...
    print(f"\n{sep}")
    print(
        df_show.to_string(index=False, na_rep="N/A", float_format=lambda x: f"{x:.4f}")
    )
    print(sep)


def _winkler_array(
    y: np.ndarray, lower: np.ndarray, upper: np.ndarray, alpha: float = 0.2
) -> np.ndarray:
...
def _significance_test(
    ...
    ws_a = _winkler_array(
        yte, all_preds[method_a]["lower"], all_preds[method_a]["upper"], alpha
    )
    ws_b = _winkler_array(
        yte, all_preds[method_b]["lower"], all_preds[method_b]["upper"], alpha
    )
...
def cmd_compare(args, ctx):
    ...
    # ── 基线 ──────────────────────────────────────────────────────
    baselines = (
        [b.strip() for b in args.baselines.split(",")] if args.baselines else None
    )
    bl_label = f"({args.baselines})" if args.baselines else "(全部)"
    print(f"\n📊 运行基线实验 {bl_label} ...")
    data = ctx["ExperimentData"](
        X_train=Xtr,
        y_train=ytr,
        X_val=Xvq1,
        y_val=yvq1,
    ...
    if not args.no_snail:
        ...
        all_preds.update(ctx["run_snail_experiment"](data, beta_values=betas))
    ...
    out_path = (
        Path(args.output)
        if args.output
        else (ctx["RESULTS_DIR"] / f"comparison_{datetime.now():%Y%m%d_%H%M%S}.csv")
    )
...
def cmd_train(args, ctx):
    ...
    if args.mode in ("snail", "all"):
        ...
        results.update(ctx["run_snail_experiment"](data, beta_values=betas))

    df_eval = ctx["evaluate_methods"](yte, results)
    _print_table(df_eval, "📈 测试集评估：")

    out_path = (
        ctx["RESULTS_DIR"] / f"train_{args.mode}_{datetime.now():%Y%m%d_%H%M%S}.csv"
    )
...
def cmd_beta_select(args, ctx):
    ...
    print(f"\n{sep}")
    print(
        f"  {'β':>8}  {'复合Score':>12}  {'MAE':>10}  {'Winkler':>10}  {'Coverage':>10}"
    )
    print(sep)
    for beta, info in beta_scores.items():
        ...
        print(
            f"  {str(beta):>8}  {m['score']:>12.4f}  {m['mae']:>10.4f}"
...
def cmd_monitor(args, ctx):
    ...
    print(
        f"  螺线参数 (log_A, B)  : ({res.get('log_A', float('nan')):.4f}, "
        f"{res.get('B', float('nan')):.4f})"
    )
    print(f"  平均外扩速度 v_t     : {mean_v:.6f}")
    print(f"  速度标准差           : {res.get('std_velocity', float('nan')):.6f}")
    print(
        f"  预警触发次数         : {alert_count} / {total_count}  (预警率 {alert_rate:.2%})"
    )

    if args.plot:
        print("\n🖼️  生成螺旋轨迹图 ...")
        ctx["plot_spiral_trajectory"](
            anchor, radius, alerts=alerts if alert_count > 0 else None
        )
    ...
    ts = {
        ...
        if len(velocity) > 0
        else np.full_like(anchor, np.nan),
...
def cmd_visualize(args, ctx):
    ...
    if vis_type in ("pareto", "all"):
        ...
        for row in df.itertuples(index=False):
            # Using getattr with default for backwards compatibility with dict-like get
        ...
        ax.set_title(
            "Pareto 曲线：区间质量对比（目标：左下角）", fontsize=14, fontweight="bold"
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    g.add_argument("--verbose", "-v", action="store_true", help="输出详细调试日志")
    g.add_argument(
        "--quiet", "-q", action="store_true", help="仅输出关键结果，抑制过程日志"
    )
    parser.add_argument(
        "--log-file", metavar="PATH", help="将日志同步写入文件（与屏幕输出并行）"
    )
    ...
    p_cmp.add_argument(
        "--no-snail", action="store_true", help="跳过蜗牛壳变体，仅运行基线对比"
    )
    p_cmp.add_argument(
        "--plot", action="store_true", help="实验完成后自动显示 Pareto + 时间动态图"
    )
    p_cmp.add_argument(
        "--output",
        type=str,
    ...
    p_ev.add_argument(
        "--results", required=True, metavar="PATH", help="结果 CSV 文件路径"
    )
...
def main():
    ...
    try:
        ...
        print(f"❌ 模块导入失败: {e}")
        print(
            "   请确认已安装 requirements.txt 中的依赖：pip install -r requirements.txt"
        )
        sys.exit(1)
