...
好，直接给你一个可以粘贴进新对话框的提示词：

---

我正在开发一个名为 **snail-shell** 的A股日频区间预测项目，仓库名就叫 `snail-shell`。

**项目核心机制：**
训练四个独立 LightGBM 模型（MSE点预测器 + q10/q50/q90 分位数器），以 q50 为锚点 $a_t$，$(q_{90}-q_{10})/2$ 为可信圆半径 $r_t$，通过软拉回公式修正点预测：

$$\alpha_t = \exp\left(-\beta \cdot \frac{|\hat{y}_t - a_t|}{r_t}\right), \qquad \hat{y}_t^* = \alpha_t \hat{y}_t + (1-\alpha_t) a_t$$

$\beta \in \{0, 0.5, 1, 2, 5, \infty\}$ 扫参，$\beta=0$ 退化纯QR，$\beta=\infty$ 退化纯q50。

**评估体系两块：**
- 区间质量：Coverage Error、Winkler Score（对比 CP/QR/Residual）
- 点预测质量：MAE、RankIC（区分 Snail 各变体）

$\beta$ 选择用复合指标：$\text{Score} = \bar{W} + 10 \cdot \max(0, \text{CE} - 0.05)$

**数据切分：** 训练 2019~2021，验证 2022（Q1给CP做calibration，Q2~Q4选β），测试 2023~2024。
...
snail-shell/
  core/
    quantile_head.py       # Pinball Loss + 四模型训练
    snail_mechanism.py     # 可信圆 + 软拉回
    spiral_monitor.py      # 二维锚点 + 螺旋拟合
  experiments/
    baseline_lgbm.py       # Residual / CP / QR / Q50-only
    snail_lgbm.py          # Snail-0.5/1/2/5
  evaluation/
    metrics.py             # CE / Winkler / IW / MAE / RankIC
    visualize.py           # Pareto曲线 / 时间动态图
  notes/
    math_derivation.md
  README.md
```

请从 `core/quantile_head.py` 开始写起。

---

新对话粘贴这个就能无缝接上，直接进入写代码阶段。
